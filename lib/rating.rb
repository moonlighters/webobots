...
module Rating
  ...
  module Winner
    ALWAYS = 5              # Очки, который победитель получает всегда
    WITH_SAME = 15          # Дополнительные очки, которые победитель получает,
                            #   побеждая кого-то такого же уровня, как он
    RISE_COEFF = 4          # Коэффициент линейной зависимости дополнительных очков
                            #   от разности, когда победитель имеет меньший рейтинг
    MAX_NEGATIVE_DIFF = 20  # Максимальное превосходство победителя над проигравшим по очкам
  ...
  def points_for(winner_points, loser_points)
    ...
    diff = (loser_points - winner_points).to_f  # Победитель получает больше, когда у проигравшего рейтинг выше!
    if diff < 0
